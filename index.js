...
server.get('/letsgetphysical', (req, res) => {
  ...
  res.render('letsgetphysical.ejs', {
    error: ""
  });
});
...
server.post('/publish', (req, res) => {
  ...
  Game.create(data, function(error, data) {
    if (error) {
      res.render("publish.ejs", {
        error: `A game with the name ${ data.title } has already been made, please reupload with a different title`
      })
      console.log(error);
    } else {
